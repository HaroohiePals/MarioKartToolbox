...
public class MemoryArchive : Archive
{
    ...
    private ArcFile GetFileByPath(string path)
    {
        ...
        return val >= 0
            ? GetDirectoryByPath(path.Substring(0, val)).GetFile(path.Substring(val + 1))
            : Root.GetFile(path);
    }

    public override IEnumerable<string> EnumerateFiles(string path, bool fullPath)
        => fullPath
            ? GetDirectoryByPath(path).Files.Select(f => f.FullPath)
    ...
    public override IEnumerable<string> EnumerateDirectories(string path, bool fullPath)
        => fullPath
            ? GetDirectoryByPath(path).Directories.Select(f => f.FullPath)
    ...
    public override void SetFileData(string path, byte[] data)
    {
        ...
        var dir = val >= 0 ? GetDirectoryByPath(path[..(val + 1)]) : Root;
        string fileName = Path.GetFileName(path);
        if (dir.ContainsFile(fileName))
            dir.GetFile(fileName).Data = data;
        else
            dir.CreateFile(fileName, data);
    }
}