...
namespace HaroohiePals.Nitro.NitroSystem.Snd.Player
{
    public static class Util
    {
        ...
        public static int SinIdx(int index)
        {
            ...
            if (index < 64)
                return SinTable[32 - (index - 32)];
            if (index < 96) 
                return -SinTable[index - 64];
            return -SinTable[32 - (index - 96)];
        }

        public static ushort CalcTimer(int baseTimer, int pitch)
        {
            int shift = 0;
            pitch = -pitch;
            ...
            ulong timer = ((ulong)PitchTable[pitch] + 0x10000ul) * (ulong)baseTimer;
            ...
            {
                ulong mask = (0xfffffffffffffffful << (32 - shift));
                if ((timer & mask) != 0)
                    return 0xffff;
                else
                    timer <<= shift;
        ...
        public static ushort CalcChannelVolume(int dB)
        {
            if (dB < -723)
                ...
                dB = 0;
            byte volume = VolumeTable[dB + 723];

            NitroChannel.DataShift shift;
            if (dB < -240)
                shift = NitroChannel.DataShift.Shift_4;
            else if (dB < -120)
                shift = NitroChannel.DataShift.Shift_2;
            else if (dB < -60)
                shift = NitroChannel.DataShift.Shift_1;
            else
                shift = NitroChannel.DataShift.Shift_0;

            return (ushort)(((int)shift << 8) | volume);
        }
    }
}