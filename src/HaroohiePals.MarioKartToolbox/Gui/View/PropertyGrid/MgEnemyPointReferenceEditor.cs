...
class MgEnemyPointReferenceEditor : IPropertyEditor
{
    protected MkdsMapData MapData;
    protected MapDataCollection<MkdsMgEnemyPath> Collection;

    public MgEnemyPointReferenceEditor(MkdsMapData mapData, MapDataCollection<MkdsMgEnemyPath> collection)
    {
        MapData = mapData;
    ...
    public bool IsEditorForProperty(PropertyInfo propertyInfo) 
        => propertyInfo.PropertyType == typeof(Reference<MkdsMgEnemyPoint>) && Collection != null;
    ...
    private static bool AreSameValues(object[] sourceObjects, string propertyName)
    {
        ...
        var firstRef =
            (Reference<MkdsMgEnemyPoint>)sourceObjects[0].GetType().GetProperty(propertyName).GetValue(sourceObjects[0]);

        object firstItem = firstRef != null ? firstRef.Target : null;

        return sourceObjects.Skip(1).All(x =>
        {
            var secondRef = (Reference<MkdsMgEnemyPoint>)x.GetType().GetProperty(propertyName).GetValue(x);
    ...
    public bool Draw(string label, PropertyEditorContext context)
    {
        ...
        var firstValue = (Reference<MkdsMgEnemyPoint>)prop.GetValue(context.Property.SourceObjects[0]);
        ...
        switch (MapDataReferenceEditor.Draw(label, sameValues ? firstValue : null, text, Collection.Select(x => x.Points), null, MapData, out var newValue))
        {
            ...
            case MapDataReferenceEditMode.UpdateAdd:
                foreach (var obj in context.Property.SourceObjects)
                {
                    if (obj is IMapDataEntry mapDataEntry)
                    {
                        ...
                        var oldRef = (Reference<MkdsMgEnemyPoint>)prop.GetValue(mapDataEntry);

                        context.TempActions.Add(new SetMapDataEntryReferenceAction<MkdsMgEnemyPoint>(mapDataEntry, context.Property.Name,
                            oldRef, newValue));
                    }
                }
