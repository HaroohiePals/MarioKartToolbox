...
public class Vector2PropertyEditor : IPropertyEditor
{
    public bool IsEditorForProperty(PropertyInfo propertyInfo) 
        => propertyInfo.PropertyType == typeof(Vector2d);
    ...
    public bool Draw(string label, PropertyEditorContext context)
    {
        ...
        var vec = (Vector2d)prop.GetValue(context.Property.SourceObjects[0]);
        ...
        AreSameValues(context.Property, out bool sameX, out bool sameY);

        vec = (sameX ? vec.X : 0, sameY ? vec.Y : 0);
        ...
        if (ImGuiEx.DragScalar($"{label}_X", vec.X, out var outPropX, sameX ? null : "", min, max))
        {
            Array.ForEach(context.Property.SourceObjects, x =>
            {
                ...
                var realValue = (Vector2d)curProp.GetValue(x);
    ...
    /// <summary>
    /// Check which components of Vec2 are different inside the entire selection
    /// </summary>
    private static void AreSameValues(PropertyGridItem property, out bool sameX, out bool sameY)
    {
        ...
        {
            double value = ((Vector2d)x.GetType().GetProperty(property.Name).GetValue(x)).X;
            return value.Equals(((Vector2d)property.SourceObjects[0].GetType().GetProperty(property.Name)
                .GetValue(property.SourceObjects[0])).X);
        });
        sameY = property.SourceObjects.Skip(1).All(x =>
        {
            double value = ((Vector2d)x.GetType().GetProperty(property.Name).GetValue(x)).Y;
            return value.Equals(((Vector2d)property.SourceObjects[0].GetType().GetProperty(property.Name)
