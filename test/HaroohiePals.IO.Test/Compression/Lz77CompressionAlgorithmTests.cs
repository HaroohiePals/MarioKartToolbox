using HaroohiePals.IO.Compression;
using NUnit.Framework;

namespace HaroohiePals.IO.Test.Compression;

class Lz77CompressionAlgorithmTests
{
    [Test]
    public void Lz77CompressionAlgorithm_Decompress_DecompressesData()
    {
        // Arrange
        var lz77 = new Lz77CompressionAlgorithm();

        // Act
        var result = lz77.Decompress(Lz77CompressedData);

        // Assert
        Assert.That(result, Is.EqualTo(UncompressedData));
    }

    [Test]
    public void Lz77CompressionAlgorithm_Compress_CompressesData()
    {
        // Arrange
        var lz77 = new Lz77CompressionAlgorithm();

        // Act
        var result = lz77.Compress(UncompressedData);

        // Assert
        Assert.That(lz77.Decompress(result), Is.EqualTo(UncompressedData));
    }

    private static readonly byte[] UncompressedData =
    {
        
    };

    private static readonly byte[] Lz77CompressedData =
    {
        
    };
}